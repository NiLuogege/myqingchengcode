...
public class PreferentialServiceImpl implements PreferentialService {
    ...
    public int findPreMoneyByCategoryId(Integer categoryId, int money) {
        ...
        criteria.andGreaterThanOrEqualTo("endTime", new Date());//截至日期大于等于现在时间
        ...
        System.out.println("preferentials=" + preferentials.toString());
        ...
        if (preferentials.size() > 0) {
            ...
            if ("1".equals(preferential.getType())) {//不翻倍
                ...
                System.out.println("preferential.getPreMoney=" + preferential.getPreMoney()+"money="+money);
                return preferential.getPreMoney() * (money / preferential.getBuyMoney());
            }
        } else {
            return 0;//没有优惠
        }
